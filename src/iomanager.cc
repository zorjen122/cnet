...
IOManager::IOManager(size_t threadSize, bool onCaller)
    : Scheduler(threadSize, onCaller), ioContext(10),
      tick(), pendingEventCount(0), epollTimeOut(2000)
{
  int rt = 0;
  ...
  for (;;)
  {
    ...
    for (int i = 0; i < rt; ++i)
    {
      ...
      if (event.events & EPOLLIN)
        triggerEventType |= EPOLLIN;
      if (event.events & EPOLLOUT)
        triggerEventType |= EPOLLOUT;

      if (triggerEventType == 0x0)
        continue;
...
void IOManager::add(epoll_event event, const std::function<void()> &callback)
{
  ...
  if (fd >= ioContext.size())
    ioContext.resize(fd * 1.5);
...
void IOManager::add(int fd, int eventType, const std::function<void()> &callback)
{
  epoll_event event;
  event.data.fd = fd;
...
{
  return Scheduler::checkStoping() && pendingEventCount == 0 && !TimerManager::hasTimer();
}
